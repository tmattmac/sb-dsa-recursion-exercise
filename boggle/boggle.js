...
function check(board, letter, i, j) {
  if (i < 0 || i >= board.length || j < 0 || j >= board[0].length)
    return false;
  return board[i][j] === letter;
}

function find(board, word, checked = new Set(), last) {
/** Can word be found in board? */
  if (checked.size === word.length) return true;
  if (checked.size) {
    const toCheck = [
      [last.i + 1, last.j],
      [last.i - 1, last.j],
    ...
    for (let coord of toCheck) {
      let [i, j] = coord;
      if (check(board, word[checked.size], i, j) &&
        !checked.has(`${i} ${j}`)) {
          const newChecked = new Set([...checked, `${i} ${j}`]);
          if (find(board, word, newChecked, {i, j})) return true;
        }
    }
  }
  else {
    for (let i = 0; i < board.length; i++) {
      for (let j = 0; j < board[0].length; j++) {
        if (board[i][j] === word[checked.size]) {
          const newChecked = new Set([...checked, `${i} ${j}`]);
          if (find(board, word, newChecked, {i, j})) return true;
        }
      }
